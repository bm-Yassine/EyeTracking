...
def main():
    ...
    prev_rvec = None
    prev_tvec = None
    prev_angles = None  # (yaw, pitch, roll)
    frame_counter = 0
    ...
    try:
        while idx < total_pts:
            ...
            if cam is not None:
                ...
                if K is not None and out.get("ok", False):
                    # roll from 2D eye-line (outer-outer)
                    ...
                    if hp is not None and getattr(hp, "ok", False):
                        ...
                        # resolve flips w.r.t. eye-line + previous, then clamp to [-50, 50]
        ...
        if 'sess_log' in locals() and sess_log is not None: sess_log.close()
